...
export function placingBet() {
    ...
    if (betAmount <= currentUser.credits) {
        ...
        for (let index = 0; index < selectedOdds.length; index++) {
            ...
            if (!result) {
                win = false;
                const prize = betAmount * selectedOdds.reduce((acc, num) => acc * num, 1);
                // Handle loss
                const newBet = {
                    result: "Loss",
                    betAmount: betAmount,
                ...
                const userIndex = users.findIndex((user) => user.name === currentUser.name &&
                    user.password === currentUser.password);
                if (userIndex !== -1) {
                    // Copy all updated properties from currentUser to the user in the array
                    users[userIndex].credits = currentUser.credits;
                    users[userIndex].history = Object.assign({}, currentUser.history);
                }
                // Update localStorage
                ...
                if (match.split("-")[1] == "draw") {
                    document.querySelector(".modal-body-losing").innerHTML = `Vesztettél!\nNem lett döntetlen!`;
                }
                else {
        ...
        if (win) {
            ...
            // Show winning modal
            document.getElementById("modalButton").click();
            document.querySelector(".modal-body-losing").innerHTML = `Nyertél!\nNyert összeg: ${prize}`;
        }
    }
}
